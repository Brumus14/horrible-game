...
public class GameArena extends JPanel
    ...
    private void init(int width, int height, boolean createWindow) {
        ...
        try {
            ...
            RenderingHints.Key key =
                (RenderingHints.Key)rh.getField("KEY_RESOLUTION_VARIANT")
                    .get(null);
            Object value =
                rh.getField("VALUE_RESOLUTION_VARIANT_DPI_FIT").get(null);
     ...
     *
     * n.b. This should only be called if this GameArena was constructed without
     * its own JFrame
     *
        ...
        synchronized (this) {
            if (!this.exiting) {
                ...
                if (backgroundImage != null)
                    graphics.drawImage(backgroundImage, 0, 0, arenaWidth,
                                       arenaHeight, 0, 0,
                ...
                for (Object o : things) {
                    if (o instanceof Ball) {
                        Ball b = (Ball)o;
                        graphics.setColor(
                            this.getColourFromString(b.getColour()));
                        graphics.fillOval(
                            (int)(b.getXPosition() - b.getSize() / 2),
                            (int)(b.getYPosition() - b.getSize() / 2),
                            (int)b.getSize(), (int)b.getSize());
                    }

                    if (o instanceof Rectangle) {
                        Rectangle r = (Rectangle)o;
                        graphics.setColor(
                            this.getColourFromString(r.getColour()));
                        graphics.fillRect(
                            (int)r.getXPosition(), (int)r.getYPosition(),
                    ...
                    if (o instanceof Line) {
                        Line l = (Line)o;
                        graphics.setColor(
                            this.getColourFromString(l.getColour()));
                        graphics.setStroke(
                            new BasicStroke((float)l.getWidth()));
                        ...
                        if (l.getArrowSize() > 0) {
                            float arrowRatio =
                                (float)(1.0 -
                                        ((l.getWidth() * l.getArrowSize()) /
                                         l.getLength()));
                            ex = sx + ((ex - sx) * arrowRatio);
                            ey = sy + ((ey - sy) * arrowRatio);
                            graphics.fillPolygon(l.getArrowX(), l.getArrowY(),
                                                 3);
                        }
                    ...
                    if (o instanceof Text) {
                        Text t = (Text)o;
                        graphics.setFont(
                            new Font("SansSerif", Font.BOLD, t.getSize()));
                        graphics.setColor(
                            this.getColourFromString(t.getColour()));
                        graphics.drawString(t.getText(),
                                            (float)t.getXPosition(),
                                            (float)t.getYPosition());
                    }
                }
            }

            window.drawImage(buffer, this.getInsets().left,
                             this.getInsets().top, this);
        }
    }

    //
    // Shouldn't really handle colour this way, but the student's haven't been
    // introduced to constants properly yet, hmmm....
    //
     ...
     * This method causes your program to delay for 1/50th of a second. You'll
     * find this useful if you're trying to animate your application.
     *
     */
    public void pause() {
        try {
     ...
     *	@param asciiVal the ascii character value to check if it is within A-Z
     *ascii.
     *	@return true or false if the ascii character value is A-Z ascii.
     */
    private boolean isAsciiAZ(int asciiVal) {
        if (asciiVal >= asciiAZOffset &&
            asciiVal <= asciiAZOffset + alphabetLength)
            return true;
    ...
    public boolean letterPressed(char key) {
        if (Character.isLetter(key))
            if (isAsciiAZ((int)Character.toUpperCase(key)))
                return alphasPressed[(int)Character.toUpperCase(key) -
                                     asciiAZOffset];
        return false;
    }
